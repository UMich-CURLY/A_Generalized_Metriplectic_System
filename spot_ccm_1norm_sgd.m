...
% x = msspoly('x', 2);
H = 1 / 2 * (x(1)^2 + x(2)^2);
S = - 1 / 2 * (x(2)^2 + 4 * x(1)^4 - 4 * x(1)^2)^2 - H;
vx_ = monomials(x, 0:8);
[~, dm, M] = decomp([vx_; S]);
p_gt = full(M(end, :));

pnum = p_gt(:) + randn(size(p_gt(:))) * 0.5;
% pnum = randn(size(pnum));

data_raw = load("linear_data.mat"); data_raw = data_raw.data;
% data = data_raw;
...
for iter = 1:100000
    ...
    pnum = sol_lp.eval(p);
    %%
    cost_sdp(iter) = double( sol_sdp.eval(sol_sdp.objective) );
    cost_lp(iter) = double( sol_lp.eval(sol_lp.objective) );
    
    [~, ~, MM] = decomp([vx; Gamma_fun(:)]);

    metric_log{iter} = MM;
    entropy_log{iter} = double(pnum);
end
%%
close all
